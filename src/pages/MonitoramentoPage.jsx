...
const CLR = { operational: '#34d399', degraded: '#fbbf24', outage: '#f87171', unknown: '#64748b' }

const NFCE_STATES = {
  SP: { name: 'São Paulo', stores: 105 },
  RS: { name: 'Rio Grande do Sul', stores: 63 },
  SC: { name: 'Santa Catarina', stores: 31 },
  BA: { name: 'Bahia', stores: 9 },
  CE: { name: 'Ceará', stores: 9 },
  ES: { name: 'Espírito Santo', stores: 5 },
  MG: { name: 'Minas Gerais', stores: 5 },
  RJ: { name: 'Rio de Janeiro', stores: 1 },
}

const OPERATIONAL_STATES = ['SP', 'RS', 'SC', 'BA', 'CE', 'ES', 'MG', 'RJ']

const ALL_STATE_PATHS = {
  TO: 'M289.558,235.641c16.104,0.575,44.973-31.647,44.835-45.259c-0.136-13.612-17.227-58.446-22.349-66.088c-5.122-7.628-37.905,2.506-37.905,2.506S234.852,233.695,289.558,235.641z',
  ...
  SE: 'M408.561,191.735c0.521-1.505,2.465-0.725,3.533-0.794c2.273-0.164,0.494-2.738,1.095-3.778c2.026-3.793-2.738-5.999-1.998-10.408c4.024,1.931,9.448,3.397,12.408,6.89c1.343,1.533,5.504,2.656,5.832,4.847c-6.822,0.384-6.901,8.819-11.942,11.572C413.545,202.212,407.055,193.721,408.561,191.735z',
...
function getChecksForService(svc, allChecks) {
  return allChecks.filter(c => c.service_id === svc.id).slice(0, 3)
}

function DarkBrazilMap({ nfceServices, checks, statusMap }) {
  function getColor(uf) {
    const d = statusMap[uf]
    if (!d) return '#1e293b'
    return CLR[d.status] || '#1e293b'
  }

  return (
    <svg viewBox="0 0 460 470" style={{ width: '100%', height: '100%' }}>
      <defs>
      ...
      {Object.entries(ALL_STATE_PATHS).map(([uf, path]) => {
        const isMonitored = OPERATIONAL_STATES.includes(uf)
        const d = statusMap[uf]
        const color = isMonitored ? getColor(uf) : '#1e293b'
        const hasIssue = d && d.status !== 'operational' && d.status !== 'unknown'
        const isUnknown = d && d.status === 'unknown'
        const [lx, ly] = STATE_LABEL_POS[uf] || [0, 0]
        const storeCount = NFCE_STATES[uf]?.stores || 0
        ...
        return (
          <g key={uf}>
            <path
              d={path}
              fill={color}
              fillOpacity={isMonitored ? (isUnknown ? 0.35 : hasIssue ? 0.85 : 0.55) : 0.25}
              stroke="#0b0f14"
              strokeWidth={hasIssue ? 2 : 1.2}
              strokeLinecap="round"
              strokeLinejoin="round"
              filter={hasIssue ? (d.status === 'outage' ? 'url(#glow-red)' : 'url(#glow-amber)') : 'none'}
              style={{ transition: 'all 0.4s' }}
            />
            {isMonitored ? (
              <g>
                <rect x={lx - 22} y={ly - 11} width={44} height={24} rx={5}
                  fill="#0f172a" fillOpacity={0.88} stroke={hasIssue ? color : isUnknown ? '#64748b' : '#1e293b'} strokeWidth={hasIssue ? 1.5 : 1} />
                <text x={lx} y={ly + 1} textAnchor="middle" fontSize={11} fontWeight={700}
                  fontFamily="Inter, system-ui" fill={hasIssue ? color : '#fff'} fillOpacity={0.95}
                  style={{ pointerEvents: 'none', userSelect: 'none' }}>
                  {uf}
                </text>
                <text x={lx} y={ly + 11} textAnchor="middle" fontSize={8} fontWeight={600}
                  fontFamily="Inter, system-ui" fill={hasIssue ? color : isUnknown ? '#64748b' : '#94a3b8'} fillOpacity={0.8}
                  style={{ pointerEvents: 'none', userSelect: 'none' }}>
                  {storeCount} lojas
                </text>
              </g>
...
export default function MonitoramentoPage({ fullscreen }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [svcRes, chkRes] = await Promise.all([
        supabase.from('services').select('*').eq('enabled', true).order('category').order('name'),
        supabase.from('service_checks').select('*').order('checked_at', { ascending: false }).limit(150),
      ])
      ...
      console.error('Erro ao buscar dados de monitoramento:', err.message)
      setConnected(false)
    } finally {
      setLoading(false)
    }
  ...
  const nfceServices = useMemo(() => services.filter(s => s.name.startsWith('NFC-e')), [services])
  const paymentServices = useMemo(() => services.filter(s => s.name === 'PIX (BACEN/SPI)' || s.name === 'Getnet'), [services])

  const tvData = useMemo(() => {
    return OPERATIONAL_STATES.map(uf => {
      const svc = nfceServices.find(s => s.name === `NFC-e ${uf}`)
      const svcChecks = svc ? getChecksForService(svc, checks) : []

      const validChecks = svcChecks.filter(c => c.latency_ms != null)
      const avgMs = validChecks.length > 0
        ? Math.round(validChecks.reduce((sum, c) => sum + c.latency_ms, 0) / validChecks.length)
        : null

      let status = 'unknown'
      if (svcChecks.length >= 1) {
        status = svcChecks[0].status
      }

      return {
        uf, name: NFCE_STATES[uf].name, stores: NFCE_STATES[uf].stores, status,
        tempo: avgMs ? avgMs / 1000 : 0,
        check: avgMs ? formatLatency(avgMs) : '—',
        checkedAt: svcChecks[0]?.checked_at,
      }
    })
  }, [nfceServices, checks])

  const paymentData = useMemo(() => {
    return paymentServices.map(svc => {
      const svcChecks = getChecksForService(svc, checks)
      const validChecks = svcChecks.filter(c => c.latency_ms != null)
      const avgMs = validChecks.length > 0
        ? Math.round(validChecks.reduce((sum, c) => sum + c.latency_ms, 0) / validChecks.length)
        : null
      ...
      const label = svc.name === 'PIX (BACEN/SPI)' ? 'PIX (BACEN)' : svc.name === 'Getnet' ? 'Getnet (Fiserv)' : svc.name
      return {
        id: svc.id, name: label, status,
        check: avgMs ? formatLatency(avgMs) : '—',
        checkedAt: svcChecks[0]?.checked_at,
      }
    })
  }, [paymentServices, checks])

  const tvSorted = useMemo(() => {
    return [...tvData].sort((a, b) => {
      const p = { outage: 0, degraded: 1, operational: 2, unknown: 3 }
  ...
  const statusMap = useMemo(() => {
    const map = {}
    tvData.forEach(s => { map[s.uf] = s })
    return map
  }, [tvData])

  const issues = useMemo(() => tvData.filter(s => s.status !== 'operational' && s.status !== 'unknown'), [tvData])
  const hasAnyIssue = issues.length > 0
  const totalLojas = tvData.reduce((a, s) => a + s.stores, 0)

  const [clockStr, setClockStr] = useState('')
  useEffect(() => {
    const tick = () => setClockStr(new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }))
    tick()
  ...
  const staleMinutes = useMemo(() => {
    const latestCheck = tvData.reduce((latest, s) => {
      if (!s.checkedAt) return latest
      const t = new Date(s.checkedAt).getTime()
      return t > latest ? t : latest
    }, 0)
    if (!latestCheck) return null
    return Math.floor((Date.now() - latestCheck) / 60000)
  }, [tvData, clockStr])

  if (loading) {
    return (
      <div style={{ background: '#0b0f14', minHeight: fullscreen ? '100vh' : 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 32, height: 32, border: '4px solid #1e293b', borderTopColor: '#34d399', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto' }} />
          <p style={{ marginTop: 16, color: '#475569', fontSize: 14 }}>Carregando monitoramento...</p>
        </div>
  ...
  const header = fullscreen ? (
    <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', height: 56, flexShrink: 0, borderBottom: '1px solid #1e293b' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg, #ef4444, #f97316)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: '#fff', fontWeight: 800 }}>S</div>
        <span style={{ color: '#f1f5f9', fontSize: 16, fontWeight: 700 }}>Store</span>
        <span style={{ color: '#f97316', fontSize: 16, fontWeight: 700 }}>Ops</span>
        <span style={{ color: '#334155', margin: '0 8px' }}>·</span>
        <span style={{ color: '#94a3b8', fontSize: 15 }}>Monitoramento</span>
      </div>

      {(() => {
        if (issues.length === 0) {
          return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 18px', borderRadius: 20, background: '#34d39910', border: '1px solid #34d39920' }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#34d399', boxShadow: '0 0 8px #34d39950' }} />
              <span style={{ color: '#34d399', fontSize: 14, fontWeight: 600 }}>{tvData.filter(s => s.status === 'operational').length}/{tvData.length} operacionais</span>
              <span style={{ color: '#334155', margin: '0 2px' }}>·</span>
              <span style={{ color: '#64748b', fontSize: 13 }}>{totalLojas} lojas</span>
            </div>
          )
        }
        const totalImpact = issues.reduce((a, s) => a + s.stores, 0)
        const worstStatus = issues.some(s => s.status === 'outage') ? 'outage' : 'degraded'
        const worstColor = CLR[worstStatus]
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 18px', borderRadius: 20, background: `${worstColor}10`, border: `1px solid ${worstColor}20` }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: worstColor, boxShadow: `0 0 8px ${worstColor}50`, animation: 'tvpulse 2s ease-in-out infinite' }} />
            <span style={{ color: worstColor, fontSize: 14, fontWeight: 600 }}>
              {issues.map(i => i.uf).join(', ')} {worstStatus === 'outage' ? 'fora' : issues.length === 1 ? 'degradado' : 'degradados'}
            </span>
            <span style={{ color: '#334155', margin: '0 2px' }}>·</span>
            <span style={{ color: '#e2e8f0', fontSize: 14, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{totalImpact} lojas impactadas</span>
          </div>
        )
      })()}
  ...
  const panel = (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, minHeight: 0 }}>
      <div style={{ borderRight: '1px solid #1e293b', padding: '16px 24px', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'auto' }}>
          {!connected && (
            <div style={{
              display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', marginBottom: 8,
              borderRadius: 10, background: '#f8717110', border: '1px solid #f8717130',
            }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#f87171', flexShrink: 0, animation: 'tvpulse 2s ease-in-out infinite' }} />
              <span style={{ color: '#f87171', fontSize: 13, fontWeight: 600 }}>
                Conexão perdida — tentando reconectar...
              </span>
            </div>
          )}
          {staleMinutes != null && staleMinutes >= 5 && connected && (
            <div style={{
              display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', marginBottom: 8,
              borderRadius: 10, background: '#fbbf2410', border: '1px solid #fbbf2430',
            }}>
              ...
              <span style={{ color: '#fbbf24', fontSize: 13, fontWeight: 600 }}>
                Dados desatualizados — última verificação há {staleMinutes} min
              </span>
            </div>
          )}
          {/* Card NFC-e / Fiscal */}
          <div style={{ borderRadius: 12, border: '1px solid #1e293b', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 20px', background: 'linear-gradient(135deg, #1e293b 0%, #172033 100%)', borderBottom: '1px solid #334155' }}>
              <span style={{ fontSize: 14 }}>📄</span>
              <span style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Fiscal — NFC-e por Estado</span>
              <span style={{ marginLeft: 'auto', color: '#64748b', fontSize: 11, fontWeight: 500 }}>{tvData.length} estados · {totalLojas} lojas</span>
            </div>
            <div style={{
              display: 'grid',
              gridTemplateColumns: hasAnyIssue ? '1fr 100px 120px 90px' : '1fr 120px 140px',
              padding: '6px 20px', background: '#141a22',
            }}>
              <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Estado</span>
              <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center' }}>Latência</span>
              <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center' }}>Checagem</span>
              {hasAnyIssue && <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center' }}>Impacto</span>}
            </div>
            <div style={{ background: '#0f1318', overflow: 'auto' }}>
              {tvSorted.map((st, i) => {
                const color = CLR[st.status] || CLR.unknown
                const isIssue = st.status !== 'operational' && st.status !== 'unknown'
                ...
                return (
                  <div key={st.uf} style={{
                    ...
                    gridTemplateColumns: hasAnyIssue ? '1fr 100px 120px 90px' : '1fr 120px 140px',
                    padding: '10px 20px',
                    background: isIssue ? `${color}08` : i % 2 === 0 ? '#111820' : '#0f1318',
                    borderLeft: `3px solid ${isIssue ? color : 'transparent'}`,
                    alignItems: 'center', transition: 'all 0.3s',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{
                        width: 8, height: 8, borderRadius: '50%', background: color,
                        boxShadow: isIssue ? `0 0 10px ${color}50` : 'none', flexShrink: 0,
                        animation: isIssue ? 'tvpulse 2s ease-in-out infinite' : 'none',
                      }} />
                      <span style={{ color: isUnknown ? '#64748b' : isIssue ? '#f1f5f9' : '#e2e8f0', fontSize: 14, fontWeight: isIssue ? 700 : 500, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{st.name}</span>
                      {isUnknown && <span style={{ color: '#475569', fontSize: 11, fontStyle: 'italic' }}>sem dados</span>}
                    </div>
                    <span style={{
                      color: st.status === 'outage' ? '#f87171' : st.status === 'degraded' ? '#fbbf24' : '#94a3b8',
                      fontSize: 14, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", textAlign: 'center',
                    }}>{st.check}</span>
                    <span style={{ color: '#64748b', fontSize: 13, fontFamily: "'JetBrains Mono', monospace", textAlign: 'center' }}>
                      {st.checkedAt ? formatTime(st.checkedAt) : '—'}
                    </span>
                    {hasAnyIssue && (
                      <div style={{ display: 'flex', justifyContent: 'center' }}>
                        {isIssue ? (
                          <span style={{ padding: '4px 12px', borderRadius: 12, background: `${color}15`, border: `1px solid ${color}30`, color, fontSize: 13, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>{st.stores} lojas</span>
                        ) : (
          ...
          {paymentData.length > 0 && (
            <div style={{ borderRadius: 12, border: '1px solid #1e293b', overflow: 'hidden', marginTop: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 20px', background: 'linear-gradient(135deg, #1e293b 0%, #1a2332 100%)', borderBottom: '1px solid #334155' }}>
                <span style={{ fontSize: 14 }}>💳</span>
                <span style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Pagamento</span>
                <span style={{ marginLeft: 'auto', color: '#64748b', fontSize: 11, fontWeight: 500 }}>{paymentData.length} serviços</span>
              </div>
              <div style={{
                display: 'grid',
                ...
                <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Serviço</span>
                <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center' }}>Latência</span>
                <span style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'center' }}>Checagem</span>
              </div>
              <div style={{ background: '#0f1318' }}>
                {paymentData.map((pm, i) => {
                  const color = CLR[pm.status] || CLR.unknown
                  const isIssue = pm.status !== 'operational' && pm.status !== 'unknown'
                  return (
                    <div key={pm.id} style={{
                      display: 'grid',
                      gridTemplateColumns: '1fr 120px 140px',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <span style={{ color: isIssue ? '#f1f5f9' : '#e2e8f0', fontSize: 14, fontWeight: isIssue ? 700 : 500 }}>{pm.name}</span>
                      </div>
                      <span style={{
                        color: pm.status === 'outage' ? '#f87171' : pm.status === 'degraded' ? '#fbbf24' : '#94a3b8',
                        fontSize: 14, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", textAlign: 'center',
                      }}>{pm.check}</span>
                      <span style={{ color: '#64748b', fontSize: 13, fontFamily: "'JetBrains Mono', monospace", textAlign: 'center' }}>
                        {pm.checkedAt ? formatTime(pm.checkedAt) : '—'}
                      </span>
                    </div>
                  )
                })}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '8px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 28, padding: '12px 28px', borderRadius: 12, background: '#0f172a', border: '1px solid #1e293b' }}>
          {[
            ...
            { c: '#f87171', l: 'Fora do ar' },
            { c: '#64748b', l: 'Sem dados' },
            { c: '#1e293b', l: 'Sem monitoramento', op: 0.5 },
          ].map(x => (
            <div key={x.l} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 10, height: 10, borderRadius: '50%', background: x.c, opacity: x.op || 0.85 }} />
              <span style={{ color: '#94a3b8', fontSize: 13, fontWeight: 500 }}>{x.l}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (fullscreen) {
    return (
      <div style={{ background: '#0b0f14', height: '100vh', fontFamily: "'Inter', system-ui, -apple-system, sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'fixed', inset: 0, zIndex: 50 }}>
        <style>{`@keyframes tvpulse { 0%,100%{transform:scale(1);opacity:1} 50%{transform:scale(1.5);opacity:0.3} }`}</style>
        {header}
        {panel}
      </div>
    )
  ...
  return (
    <div style={{ background: '#0b0f14', borderRadius: 16, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden', height: 'calc(100vh - 120px)', minHeight: 500 }}>
      <style>{`@keyframes tvpulse { 0%,100%{transform:scale(1);opacity:1} 50%{transform:scale(1.5);opacity:0.3} }`}</style>
      {panel}
    </div>
  )
